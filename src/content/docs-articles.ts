...
- 想跑 SEO？看 [How to Build a Landing Page with AI in 5 Minutes](/docs/build-landing-page-with-ai-in-5-minutes) 里关于 meta 和 JSON-LD 的内容。`,
  ...
  {
    slug: "api",
    title: "REST API — Programmatic UI Generation（REST API 编程生成）",
    description:
      "Use TensorView Builder REST API and API keys to generate React UI from scripts or CI. POST /api/v1/generate.",
    keywords: "TensorView API, AI UI generator API, REST generate, API key",
    datePublished: "2026-06-13",
    dateModified: "2026-06-13",
    readMinutes: 4,
    body: `# REST API 编程生成 UI

TensorView Builder 提供 **POST /api/v1/generate**，与网页编辑器使用同一套 AI 引擎。

## 认证

在 **设置 → API Keys** 创建 Key，请求头：

\`\`\`
X-API-Key: tv_xxxxxxxx
\`\`\`

或使用 Supabase 会话 JWT：\`Authorization: Bearer <token>\`
...
curl -X POST https://ai.tensorview.cc/api/v1/generate \\
  ...
  -H "X-API-Key: tv_your_key" \\
  -d '{"projectId":"YOUR_UUID","prompt":"SaaS landing page, dark theme","styleId":"minimal-saas"}'
\`\`\`

可选 \`styleId\`：\`minimal-saas\` | \`dark-dev\` | \`warm-commerce\` | \`corporate\` | \`playful\`

## 响应

返回 \`sandpack\` 路由与文件树，可直接用于预览或导出 Vite 项目。

## 本地 Ollama

\`docker compose up -d ollama\` 后设置 \`OLLAMA_BASE_URL\` 与 \`OLLAMA_MODEL\`。

完整说明见仓库 [docs/API.md](https://github.com/qlghmz/ai-tensorview-cc/blob/main/docs/API.md)。`,
  },
];
