...
- 实时预览，对话式持续优化
- 一键发布到自定义域名
- 内置 Lovable Cloud（数据库 + 鉴权），不用自己搭后端
...
点击右上角「发布」，再到「设置 → 自定义域名」绑定你的域名。CNAME 解析到 Lovable 提供的地址即可，平台会自动签 HTTPS 证书。
...
发布后会出现一个公开 URL，比如 \`https://your-project.lovable.app\`。在隐身窗口里打开它，确认：
...
\`*.lovable.app\` 临时域名能跑通，但要做正经品牌，得绑你自己的域名（比如 \`yourbrand.com\`）。这篇是最完整的 AI web builder custom domain setup 指南。

## 准备工作

- 一个已经发布到 \`*.lovable.app\` 的项目
